...
# This registers whole bunch of custom omega conf resolvers
import lib
...
def launch_rlg_hydra(cfg: DictConfig):
    ...
    from lib.rl.network_builder_residual_sh import (
        ResRHDictObsBuilder,
        ResLHDictObsBuilder,
    )
    ...
    def create_isaacgym_env():
        ...
        if not cfg.headless:
            assert "pcd" not in cfg.task_name.lower(), (
    ...
    if cfg.test:
        ...
        cfg.rl_train.params.config.full_experiment_name = experiment_dir.replace(
            "runs/", ""
        )
        runner.params["config"]["full_experiment_name"] = experiment_dir.replace(
            "runs/", ""
        )
...
EVAL = [
    ...
    "rh_base_model_checkpoint=imitator_ckp/imitator_rh_inspire.pth",
    "lh_base_model_checkpoint=imitator_ckp/imitator_lh_inspire.pth",
    "actionsMovingAverage=0.4",
    "dataIndices=[0001]",
    "checkpoint=runs/ResDexHand__10-23-21-06-20/nn/ResDexHand.pth",
]
